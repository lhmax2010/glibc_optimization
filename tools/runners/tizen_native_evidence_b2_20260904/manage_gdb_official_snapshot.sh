...
if [ "$mode" = remove ]; then
    ...
    run_remote GDB_REMOVE_VERIFY 'for p in gdb python3 python3-base libpython3_141_0 libgmp gdbm; do rpm -q "$p" >/dev/null 2>&1 && exit 1; done; exit 0' || exit 1
    exit 0
fi
...
# The dated baseline has none of these packages.  Refuse to acquire ownership
# of a pre-existing install, because the paired remove action must not delete it.
run_remote GDB_ABSENT_BASELINE 'for p in gdb python3 python3-base libpython3_141_0 libgmp gdbm; do rpm -q "$p" >/dev/null 2>&1 && exit 1; done; exit 0' || {
    echo "one or more GDB packages predate this round; stop instead of removing them later" >&2
    exit 1
}
...
available=$(sdb -s "$serial" shell "df -B1 / | tail -n 1 | tr -s ' ' | cut -d ' ' -f 4" </dev/null 2>/dev/null | tr -d '\r')
