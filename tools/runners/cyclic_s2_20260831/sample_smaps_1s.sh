...
while [ -r "$proc_root/$pid/smaps" ]; do
    ...
    if [ "$#" -ne 2 ]; then
        [ ! -d "$proc_root/$pid" ] && break
        sampler_rc=5
        failure_reason=stat-parse
        break
    fi
    minflt=$1
    majflt=$2
    case "$minflt:$majflt" in
        *[!0-9:]*)
            sampler_rc=5
            failure_reason=stat-parse
            break
    ...
    classes=$(awk '
      ...
      /^[0-9a-fA-F]+-[0-9a-fA-F]+ [rwxps-][rwxps-][rwxps-][rwxps-]/ {
        flush_mapping()
        split($1,a,"-"); start=("0x" a[1])+0; end=("0x" a[2])+0; perms=$2
...
if [ "$sample" -eq 0 ] && [ "$sampler_rc" -eq 0 ]; then
    sampler_rc=7
    failure_reason=zero-samples
fi
...
end_iso=$(date -Ins)
{
    ...
    printf 'RC=%s\n' "$sampler_rc"
    if [ "$sampler_rc" -eq 0 ]; then
        printf 'DONE_EXTERNAL_SAMPLER\n'
    else
        printf 'FAIL_EXTERNAL_SAMPLER\n'
    fi
} >"$meta"
