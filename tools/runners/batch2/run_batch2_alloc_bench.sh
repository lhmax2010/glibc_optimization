...
smoke_test() {
  mkdir -p "$OUT/smoke"
  sdb_shell_capture "$OUT/smoke/run.log" "rm -rf /root/alloc_batch2_smoke; mkdir -p /root/alloc_batch2_smoke; env -u GLIBC_TUNABLES '$REMOTE_BIN' --profile small-churn --threads 2 --seed 1 --warmup 0 --ops-per-thread 1000 --idle 0 --outdir /root/alloc_batch2_smoke > /root/alloc_batch2_smoke/result.json 2> /root/alloc_batch2_smoke/stderr.txt; rc=\$?; echo \$rc > /root/alloc_batch2_smoke/exit_code.txt; echo EXIT=\$rc"
