...
output=
gbs=
remote=/opt/usr/glibc_memopt/gbs_heldout_validation_20260904
contract="$repo/tools/runners/gbs_heldout_validation_20260904/contract.json"

usage() { echo "usage: $0 --ip <address> --output <host-dir> --gbs <ELF>" >&2; }
while [ "$#" -gt 0 ]; do
    case "$1" in
        --ip) ip=$2; shift 2;;
...
[ -n "$ip" ] && [ -n "$output" ] && [ -f "$gbs" ] || { usage; exit 2; }
...
remote_recovery()
{
    ...
    run_remote RECOVERY "test '$remote' = /opt/usr/glibc_memopt/gbs_heldout_validation_20260904 && { [ ! -e '$remote' ] || find '$remote' -depth -delete; } && for p in /sys/devices/system/cpu/cpu[0-3]/cpufreq/scaling_governor; do printf '%s\\n' schedutil >\"\$p\" || exit 1; done; rmdir /opt/usr/glibc_memopt 2>/dev/null || test ! -d /opt/usr/glibc_memopt" "$output/recovery.txt" || true
}
...
contract_expected=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["artifact"]["sha256"])' "$contract") || die "contract GBS SHA"
[ "$gbs_expected" = "$contract_expected" ] || die "manifest/contract GBS SHA mismatch"
[ "$(sha_of "$gbs")" = "$gbs_expected" ] || die "host GBS SHA"
{
    printf 'artifact\tsha_source\tsha256\n'
    printf 'alloc_bench.armv7l\tgbs_build_sha256\t%s\n' "$gbs_expected"
...
run_remote CREATE_WORKDIR "test ! -e '$remote' && mkdir -p '$remote/bin'" "$output/create_workdir.txt" || die "workdir"
sdb -s "$serial" push "$gbs" "$remote/bin/alloc_bench.armv7l" </dev/null >"$output/push_gbs.txt" 2>&1 || die "push GBS"
sdb -s "$serial" push "$repo/tools/runners/gbs_heldout_validation_20260904/run_heldout_remote.sh" "$remote/run_heldout_remote.sh" </dev/null >"$output/push_runner.txt" 2>&1 || die "push runner"
...
run_remote ASSET_VERIFY "chmod 0755 '$remote/bin/alloc_bench.armv7l' '$remote/run_heldout_remote.sh' '$remote/sample_smaps_1s.sh' && test \$(sha256sum '$remote/bin/alloc_bench.armv7l' | awk '{print \$1}') = '$gbs_expected' && test \$(sha256sum '$remote/medium_1k_16k.hist' | awk '{print \$1}') = 2082e156db133f4e6e900aec7c202e44a453d2f23b60225c40251de08a27960b" "$output/asset_verify.txt" || die "asset verification"
run_remote HELDOUT_INVOKE "EXPECTED_GBS_SHA='$gbs_expected' sh '$remote/run_heldout_remote.sh'" "$output/remote_invoke.txt" || die "remote controller"
grep -F DONE_HELDOUT_CONTROLLER "$output/remote_invoke.txt" >/dev/null || die "controller marker"
...
python3 "$repo/tools/runners/gbs_heldout_validation_20260904/analyze_heldout.py" --pull "$output/board_pull" --contract "$contract" --output "$output/derived" >"$output/analyze.txt" 2>&1 || die "analysis/health gate"
...
run_remote WORKDIR_CLEANUP "test '$remote' = /opt/usr/glibc_memopt/gbs_heldout_validation_20260904 && find '$remote' -depth -delete && test ! -e '$remote'" "$output/cleanup.txt" || die "workdir cleanup"
...
verdict=$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["verdict"])' "$output/derived/decision.json") || die "decision read"
printf 'OVERALL\tPASS\nHELDOUT_VERDICT\t%s\nRESULTS\t%s\n' "$verdict" "$output"
