...
capture_process()
{
    ...
    {
        echo "lane=$lane point=$point epoch=$(date +%s) uptime=$(cut -d ' ' -f1 /proc/uptime) pid=$pid"
...
capture_system()
{
    point=$1
    {
        echo "point=$point epoch=$(date +%s) uptime=$(cut -d ' ' -f1 /proc/uptime)"
...

{
    ...
    echo 'lanes=8 stagger_seconds=1'
...
for lane in 1 2 3 4 5 6 7 8; do
    ...
    eval pid$lane=$pid
    {
        ...
        echo "launch_epoch=$(date +%s) launch_uptime=$(cut -d ' ' -f1 /proc/uptime)"
    } >"$laneout/run_record.txt"
    wait_marker "$laneout/program_stdout.txt" 'cycle=0 state=PROCESS_READY' 50
    ready_rc=$?
    echo "ready_marker_rc=$ready_rc" >>"$laneout/run_record.txt"
...
for lane in 1 2 3 4 5 6 7 8; do
    eval pid=\$pid$lane
    wait_marker "$out/lane$lane/program_stdout.txt" 'cycle=1 state=PLAYING_START' 150
    rc=$?
    echo "playing1_marker_rc=$rc" >>"$out/lane$lane/run_record.txt"
    test "$rc" -eq 0 || exit 21
done
...
for lane in 1 2 3 4 5 6 7 8; do
    ...
    malloc_info_only "$lane" "$pid" T1b || echo "MALLOC_INFO_T1B_FAILED lane=$lane" >>"$out/anomalies.txt"
done

for lane in 1 2 3 4 5 6 7 8; do
    wait_marker "$out/lane$lane/program_stdout.txt" 'cycle=1 state=NULL_DONE' 200
    rc=$?
    echo "null1_marker_rc=$rc" >>"$out/lane$lane/run_record.txt"
    test "$rc" -eq 0 || exit 22
done
sleep 5
...
for lane in 1 2; do
    eval pid=\$pid$lane
    wait_marker "$out/lane$lane/program_stdout.txt" 'cycle=2 state=PLAYING_START' 500
    rc=$?
    echo "playing2_marker_rc=$rc" >>"$out/lane$lane/run_record.txt"
