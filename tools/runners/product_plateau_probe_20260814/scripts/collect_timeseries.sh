...
cat >"$targets" <<'EOF'
target	comm	baseline_pid	appid
AppProcD	AppProcD	543	AppD
AppProcC	AppProcC	2671	AppB
AppProcB	AppProcB	945	AppS
ServiceA	ServiceA	3953	-
ServiceD	ServiceD	953	AppF
ServiceE	ServiceE	545	-
ServiceL	ServiceL	579	AppY
ServiceH[ServiceK]	ServiceH	3740	AppC
ServiceB	ServiceB	692	-
ServiceC	ServiceC	676	-
EOF
...
{
    second=$1
    if [ "$second" -lt 90 ]; then echo P0
    elif [ "$second" -lt 150 ]; then echo R1
    elif [ "$second" -lt 210 ]; then echo R2
    elif [ "$second" -lt 270 ]; then echo R3
    elif [ "$second" -lt 330 ]; then echo R4
    elif [ "$second" -lt 390 ]; then echo R5
    elif [ "$second" -lt 510 ]; then echo P1
    elif [ "$second" -lt 570 ]; then echo R6
    else echo P2
    fi
}
...
while [ "$sample" -lt "$samples" ]; do
    ...
    {
      IFS= read -r header
      while IFS='	' read -r target comm baseline_pid appid; do
        idx=$((idx + 1))
        (
          if [ "$appid" = "-" ]; then
