...
REMOTE_ROOT=/root/alloc_bench_batch2
...
cleanup_board_tmp() {
  mkdir -p "$OUT/restore"
  sdb_shell_capture "$OUT/restore/tmp_cleanup.log" "rm -rf '$REMOTE_ROOT' '$REMOTE_BIN' '$REMOTE_INV' /root/batch2_pre_inventory.tsv /root/batch2_pre_inventory_summary.txt /root/batch2_post_inventory.tsv /root/batch2_post_inventory_summary.txt /root/alloc_batch2_smoke /tmp/alloc_bench_batch2 /tmp/alloc_bench.armv7l /tmp/tizen_memopt_inventory.sh /tmp/batch2_pre_inventory.tsv /tmp/batch2_pre_inventory_summary.txt /tmp/batch2_post_inventory.tsv /tmp/batch2_post_inventory_summary.txt /tmp/alloc_batch2_smoke; echo ---ROOT---; ls -la /root | sed -n '1,120p'; echo ---TMP---; ls -l /tmp | sed -n '1,120p'"
}
...
pre_inventory() {
  ...
  sdb_shell_capture "$OUT/precheck/inventory_run.log" "chmod +x '$REMOTE_INV'; sh '$REMOTE_INV' > /root/batch2_pre_inventory.tsv 2> /root/batch2_pre_inventory_summary.txt; echo EXIT=\$?"
  sdb_capture "$OUT/precheck/pull_inventory_tsv.log" pull /root/batch2_pre_inventory.tsv "$OUT/precheck/"
  sdb_capture "$OUT/precheck/pull_inventory_summary.log" pull /root/batch2_pre_inventory_summary.txt "$OUT/precheck/"
...
post_inventory() {
  ...
  sdb_shell_capture "$OUT/restore/inventory_run.log" "chmod +x '$REMOTE_INV'; sh '$REMOTE_INV' > /root/batch2_post_inventory.tsv 2> /root/batch2_post_inventory_summary.txt; echo EXIT=\$?"
  sdb_capture "$OUT/restore/pull_inventory_tsv.log" pull /root/batch2_post_inventory.tsv "$OUT/restore/"
  sdb_capture "$OUT/restore/pull_inventory_summary.log" pull /root/batch2_post_inventory_summary.txt "$OUT/restore/"
}
...
smoke_test() {
  mkdir -p "$OUT/smoke"
  sdb_shell_capture "$OUT/smoke/run.log" "rm -rf /root/alloc_batch2_smoke; mkdir -p /root/alloc_batch2_smoke; env -u GLIBC_TUNABLES '$REMOTE_BIN' --profile small-churn --threads 2 --seed 1 --warmup 0 --ops-per-thread 1000 --idle 0 --outdir /root/alloc_batch2_smoke > /root/alloc_batch2_smoke/result.json 2> /root/alloc_batch2_smoke/stderr.txt; echo EXIT=\$? > /root/alloc_batch2_smoke/exit_code.txt; cat /root/alloc_batch2_smoke/exit_code.txt"
  sdb_capture "$OUT/smoke/pull.log" pull /root/alloc_batch2_smoke "$OUT/smoke/"
  if [ ! -f "$OUT/smoke/result.json" ]; then
    exception "smoke failed: result.json missing"
    return 1
  fi
  if ! python3 -m json.tool "$OUT/smoke/result.json" > "$OUT/smoke/result.pretty.json" 2> "$OUT/smoke/json_tool.err"; then
    exception "smoke failed: JSON parse failed"
    return 1
  fi
  if [ -s "$OUT/smoke/stderr.txt" ]; then
    exception "smoke stderr non-empty; see $OUT/smoke/stderr.txt"
  fi
  if ! grep -q '^0$' "$OUT/smoke/exit_code.txt"; then
    exception "smoke failed: exit code $(cat "$OUT/smoke/exit_code.txt")"
    return 1
  fi
  log "smoke ok"
}
