...
# `malloc_info` 整页容量估算器验证（2026-09-04）

> 2026-09-07 按终审裁决订正：与 B2 的实际执行日统一为 2026-09-04；本报告及紧凑件
> 路径的 `20260905` 保留为历史任务标识，不代表执行日期。估算是该轮 host 复算，
> 不是次日的新测量；输入 XML、验证数字和结论不变。
