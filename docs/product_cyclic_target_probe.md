...
> **2026-09-01 撤销/修正追注（原文保留）：** §3.2 的逐轮 `fall_edge`
> `4.166497–24.460553 s` 和中位 `19.683240 s` 可复算，但旧算法把“首次跌出
> 90% 峰值带”到“尾窗最小值落点”的等待时间计入下降沿，故撤销其作为释放
> 时长的解释；`peak_band` 也受同一离散右边界影响，不再作为真实峰值驻留时长。
> 正式替代口径是 peak 后首次观测达到 `PD <= valley + 5% × (peak−valley)`
> 的延迟上界：
> `5.223693–8.910626 s`，中位 `8.179733 s`。这是标称 1 s 序列的采样上界，
> 不是精确机制时长。逐轮在旧 `fall_start` 点已释放比例为
> `99.632503/99.679693/93.364269/58.957460/47.817460/32.686084/98.522622/55.662983%`，
> 因而塌落形态异质，不再称“一步塌回”。peak/valley 幅度、valley 水平及
> P0 `3464` → R8 `4252 kB`（`+788 kB`）不受该时长伪影影响。归因与影响面
> 详见 [`cyclic_fall_mechanism_attribution_v2_20260901.md`](cyclic_fall_mechanism_attribution_v2_20260901.md)。
