...
## 9. 2026-09-17 不落盘采样续跑

### 9.1 方法变更与事前合同

PM 授权以 host 驱动的短读取替代被镜像拒绝的未签名脚本，不申请签名、不绕过
镜像校验。本轮不推送任何文件，也不在板端保存输出；授权仍仅限本轮只读 root
会话，提权前后记录 id，成功或停止均执行 root off 并核验 UID=5001。
历史 §8 的签名停止与脚本清理记录保留，不将其改写为采样成功。

新增[不落盘合同](../tools/runners/product_floor_reconfirm_20260916/diskless_contract.json)
与[实际时刻分析器](../tools/runners/product_floor_reconfirm_20260916/analyze_diskless.py)
先提交、打 annotated tag `product-floor-diskless-contract-20260917` 并推送，
与首次连接间隔至少 600 s。旧合同与历史分类器字节不变。

候选固定为 §8 的三目标与 Top10 并集 11 个进程。先确认 PID/comm/start_ticks；
已重启者仅按唯一同名进程重新发现并记录，采样中重启则停止。每候选实际覆盖至少
600 s。初始目标 1 s；最近 10 个完整批次的 start-to-start 间隔中位超过 1.5 s，
只降级一次至目标 2 s，保留此前全部点与变更原因，不补点、不插值、不追赶突发采样。
每条请求含 RC/DONE/FAIL 不超过 200 字节，host 保存读前/读后墙钟与单调时钟。
不同进程和全局量不是同一瞬间快照，逐点记录实际区间与偏移。

仅完整 smaps 读取超过 20 s 时，允许对该候选改用一条短 awk 读取映射头与
Private_Dirty 字段，再由 host 原分桶器汇总堆、其他匿名、文件映射及总 PD；不传输
完整 smaps 的其余字段，仍保留所需 PD 交叉核对，明确标注读取范围变化。
...
host 收线：补入两项本轮公开停止回执测试后，共 105 项相关测试通过；干净 main
