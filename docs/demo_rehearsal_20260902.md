...
- 每个板上轮次记录 stability-monitor 前后告警清单与计数；v2 实际观测且按固定合同登记
...
| B medium-only 三重复中位回收/released | `84.446566%` | `84.446566%` | 锚定发布值 `84.446566% ±5 pp` | PASS；但 rep2 单格中位为 `68.169197%`，不用单格代替固定合同的三重复中位规则 |
...
两件告警匹配首条登记且已完成归档/清理，因此为 `EXPECTED` 通过。两套结论对应不同
规则版本，不互相覆盖。
...
| none p99 重复离散 | `6.784167 ms` | `5.903519 ms` | 固定合同比较带 | 用于上行判定 |
