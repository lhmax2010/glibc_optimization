...
仅对客口径、文档与显示回归，不改任何测量数据、合同、验收带或原派生文件。
...
| N12-05 | **未闭合**；实现/回归在候选中通过，main 入口恢复原字节 | 固定 Base HTTP 404，新入口 GBS 自证未取得；不放宽 current-proof 门 |
...
本次执行在新入口 GBS 自证处停止，尚未切出 demo-v13；其余修正验证后推 main，
旧 demo-v12 不动。不能将这份候选变更清单误作新版本已交付。
