...
## v12 → v13：对客口径订正（2026-09-15）

仅对客口径订正与文档，不改任何测量数据、合同、验收带或原派生文件。
修复提交与验证见[逐项闭环](demo_v13_delivery_20260915.md)，裁决见[PM 台账](pm_decisions.md)。

| 发现编号 | 变更 | 对应入口 |
|---|---|---|
| N12-01 | G3 cycle=1 与全周期池分列、收益与代价窗口来源明确 | HTML system-effect / 系统报告 / 双语 README / builder 断言 |
| N12-02 | 系统净效应与 G4 RSS 极差及 NOT-DETECTED 就地显示 | 同上与跨载体测试；原 gst 方向不改 |
| N12-03 | enlightenment 同目标只引用 272/36 KiB，8–20 KiB 属官方 GST | 相关报告文字；HTML boundaries 原文保留 |
| N12-04/V12-3 | 缺历史对象提供 fetch 恢复命令，明确公开复算 vs 完整 verify | host 诊断负控、README / 指南 |
| N12-05 | **WITHDRAWN-BY-PM**，不再修复显示 nit，入口及 current-proof 保持 v12 原字节 | 固定 Base HTTP 404；不更换已钉源、不触发 ELF 重新验证 |
| V12-5/6、N12-07 | max 字段重复中位注释、计数订正、全周期与首周期区分 | accepted_matrix README / 周末报告 / 独立复算测试 |
| 正式 release 台账 | 扫描范围、历史地址、噪声、历史 INDEX 与独立 analyzer 身份边界 | PM 台账，不放宽任何门 |

demo-v12 保留；demo-v13 仅在新 main 完整快照上替换双语入口并打 annotated 标签，
继续保留更强的三克隆 × 六环境 18 次完整 verify 与 129 次启动注入拒绝检查。
2026-09-15 续裁决解除此前 STOP_GBS_PINNED_BASE_UNAVAILABLE：Base 20260813.050338
repomd 404、Unified 仍可用，仅新建 GBS 构建证明受影响，既有证明及数字不受影响。
...
## v13 → v14：回执登记与文案口径订正（2026-09-15）

| 发现编号 | 变更 | 验证 |
|---|---|---|
| CC N13-01 / Kimi V13-1 | 本轮自行重跑完整 v13 交付矩阵，更新机器回执；首次回执另存、partial 添加 superseded 指针 | predelivery_check 原入口，三克隆 × 六环境及 129 项启动拒绝，结果见交付续节 |
| Codex N13-01 | 全周期概述 41.8/45.2/16.0；G2 首周期 45.322245% 与全周期 45.177290% 分开 | builder 从整数 KiB 独立断言，跨载体正控与变异负控 |
| Kimi V13-2 | G2 极差末位 1 ulp 双舍入差进入正式 release 建议台账 | 不改现值或任何派生数据 |
