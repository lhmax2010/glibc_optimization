...
同板性说明（2026-09-07，F12）：同一轮中的对照格由同一受控 workflow 会话串行执行，
有连续运行时间线与逐次身份/环境门记录；跨轮“同板”沿用交付方对测试板的指认。
公开件未归档可独立认证物理设备的唯一硬件序列号，因此环境指纹相同或 IP 相同本身
不能证明跨轮为同一块物理板；不把该局限提升为新的同板证明。
