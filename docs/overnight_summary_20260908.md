...
对应事前执行器修复 `d0c4247`；两者均已推 main。结果与停止证据提交为
`60bea7c63ca2c603351e0ef25df15546e78db05c`，已推 origin/main；在该干净提交上运行
普通 verify（无 dirty override）为 OVERALL PASS。本条纯文档回执自身的最终 SHA 以 Git
历史和最终响应为准，避免自引用。合同提交/tag 对象与 demo-v11 SHA 均沿用下方记录。
