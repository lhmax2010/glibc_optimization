...
后续全仓扫描发现既有 TCP 编码地址未脱敏，交付再 STOP；不影响已完成的收尾与
已验收 21 格，不重跑、不自行改写旧证据，正式交付保持 demo-v11。
...
|---|---|---|---|---|
| G1 mixed | 13.015625→7.718750 | 5.296875 / 40.696279% | **−0.167969**；极差 9.394531；NOT-DETECTED | 1.458574（释放点） |
| G2 medium-only | 13.152344→7.191406 | 5.960938 / 45.322245% | +5.304688；极差 8.136719；NOT-DETECTED | 1.478167（释放点） |
| G3 gst | 8.671875→6.859375 | 1.820312 / 21.009919%；G3 为 51 周期负载，此处为 cycle=1；全周期降幅中位 16.038164%（13.28–21.04%） | +1.855469；极差 2.816406；NOT-DETECTED | 0.843612（释放点） |
| G4 enlightenment | 11.605469→11.601562 | 0.003906 / 0.033659%；RSS 降幅极差 0.089844 MiB；NOT-DETECTED | NA，无 none 臂 | **1899.209517（含 gdb/ptrace 注入）** |

2026-09-15 续订正（Codex N13-01）：全周期口径为 G1/G2/G3 的 41.8% / 45.2% / 16.0%。
G2 首周期 45.322245%，全周期 45.177290%（两周期 × 三重复的 6 点），不是同一窗口。
来源：[原逐周期](../data/raw/system_level_before_after_20260908/accepted_matrix/cycles.tsv)、
[L1 复算](demo_reproduction_guide_20260901.md#l1-system-before-after)。不改原 TSV/JSON。

2026-09-15 对客口径订正（N12-01/02）：G3 全周期中位与范围是三重复共 153 个点的
合并统计（完整范围 13.282648–21.043165%），不是 51 个逐周期中位的再中位。
cycle=1 中位在 51 个逐周期三重复中位中最高。收益头条按合同取 cycle=1；代价按
gst_comparison.json 的 primary_cycles="2-51" 排除首周期，两者窗口不同，首周期
收益不能作为典型持续周期收益。原合同、数据、派生文件不变。

系统净效应与 G4 RSS 使用与 gst p99 相同的幅度/重复离散原则：仅当
**|中位| > 重复极差** 才判可见，以上均 NOT-DETECTED，不是统计显著性检验。
gst 原规则针对正向劣化，本处针对双向系统变化取绝对值，不修改 gst 方向规则。
因此不能把 +5.304688 MiB 解读为已证明的系统净增。
...
与[历史 B/B2](tizen_native_evidence_20260904.md)同目标 enlightenment 的 272 KiB / 36 KiB 同向，
