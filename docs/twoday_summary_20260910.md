...
2026-09-11 周末后续已全部完成，当前有效快照 **demo-v12**。
见[周末总结与完整 refs](weekend_summary_20260914.md)；本文件以下是各阶段原始状态，
不抹除当时 STOP，也不把 21 格写成一次连续新运行。

2026-09-11 周末续报：[TCP 定向脱敏与扫描器加固](weekend_redaction_20260911.md)
已获 PM 授权并闭合；下文“最新终态”均为相应阶段历史记录。新的切库结果另行汇总。
...
双文件哈希/原字节校验；无 tag 克隆通过，浅克隆缺对象明确失败并提示获取方式。
runner 244 项与 clean main 完整 verify 通过；原 board preflight、合同与 21 格未改。
四个非空目录保留为已知非阻断残留；本轮没有任何板端操作。交付矩阵尚待完成，
有效快照暂仍 demo-v11；最终结果记录到[交付验收](demo_v12_delivery_20260911.md)。
以下此前“当前终态”均保留为对应执行阶段的历史，不改写其失败证据。
...
| 构建原样证明与 host STOP 收线 | `8f7cec692904c57a2f15cbb005d5504a97407701` |
...
推送留痕：上述提交已直接推入 origin/main；本行所在后续文档提交仅补写推库状态。
停止后仅做发布完整性检查：相关本地链接 657 项通过、159 个变更文件脱敏零命中，
合同/analyzer 与原 tag 逐字节不变、manifest/验收带未改；没有重试交付矩阵。
原始日志的空格和终端转义按原文保留，不将其格式告警当成证据删改理由。
